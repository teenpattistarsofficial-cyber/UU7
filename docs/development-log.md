...
### FAQ page search
- Added `components/site/faq-search.tsx` — a client-side instant filter over `/faq`'s already-fetched groups, same pattern as `CategoryGuideSearch`: the full FAQ set is small enough to already be in memory, so filtering it as the visitor types beats a server round-trip. Matches against both question and answer text. Category sections with no remaining matches are hidden rather than rendered empty, and a match count shows while a search is active. `app/(site)/faq/page.tsx` now delegates its whole grouped-accordion render to this component; the page's own "no FAQs published at all" empty state is unchanged and distinct from the component's own "no search results" one.

### Deferred third-party analytics scripts off the critical render path
- PageSpeed audits kept showing LCP stuck in "needs improvement" (~3.6s) despite confirmed-fast cached server responses (~500-700ms `HIT`) — everything server/caching-side was solid, so the remaining lever was client-side. `app/(site)/layout.tsx` rendered `headScripts`/`footerScripts` (GA4 + Ahrefs Analytics, confirmed live) via a raw `dangerouslySetInnerHTML` div as the literal first child of `<body>`, no `next/script` optimization at all — both scripts already had `async` (ruling out the simplest theory), but were still unmanaged by Next's script loader.
- The `headScripts`/`footerScripts` admin feature is deliberately unsanitized free-text (any raw HTML), so the fix couldn't add a schema/format constraint. Added `lib/site/parse-injected-scripts.ts` using `happy-dom` (already an installed dependency via `@tiptap/html/server`, no new package) to split the pasted HTML into external `<script>` tags, inline `<script>` blocks, and any leftover non-script markup (e.g. a `<noscript>` GTM fallback). Rewrote `components/site/custom-scripts.tsx`'s `InjectedScript` to render each script via `next/script` with `strategy="afterInteractive"` (Next's own documented recommendation for analytics/tag-manager scripts — defers off the critical path without risking under-counted pageviews the way `lazyOnload` would).
- No schema/admin-UI/action changes needed — only the rendering component changed.

### Publish-time guard against invalid-category internal links, plus closing a CTA blind spot
- A fresh health-check crawl found two more `/blog/<slug>` links even after `scripts/fix-blog-prefix-links.ts` had already run once. Confirmed the tree-walker itself has no bug (it correctly recurses through every Tiptap node shape); the real issue is the script is a one-time cleanup with no ongoing enforcement — the same external `/api/publish` agent pipeline that produced the original 19 can freely reintroduce the mistake on any future publish.
- Also found a separate, silent blind spot: `post_ctas.buttonUrl` is a distinct table the original cleanup script never touched, but it renders as a real crawlable `<a href>` (`components/article/cta-block.tsx`) — a bad CTA link would never surface in a crawl-based health check the way a body-content link does.
- Extracted the tree-walk (previously copy-pasted across `lib/seo/score.ts`, `lib/editor/citations.ts`, and the fix script) into a shared `lib/editor/links.ts` (`collectLinkHrefs`). Added `lib/seo/validate-internal-links.ts` (`isInvalidInternalLink`/`findInvalidInternalLinks`) — deliberately takes the valid-prefix set as a parameter rather than hardcoding against `SITE_CATEGORIES`, since that list is explicitly a fixed set of five nav categories, not the live `categories`/`pages` tables (an editor-created category or CMS page wouldn't be in it) — validating against the static list directly would have rejected perfectly valid links. `app/api/publish/route.ts` now builds the valid-prefix set from a live DB query (all categories + all pages + `authors`/`faq`) and rejects the request with `400` + the bad links if `content` or `cta.buttonUrl` contains one, before any DB write.
- Scoped this to the publish API only, not the admin editor's `syncPostCtasByItems` — that path already has a human reviewing the link, and threading a DB query into every admin post save for a problem that's only ever shown up via the machine pipeline felt like disproportionate scope creep.
- Extended `scripts/fix-blog-prefix-links.ts` to also scan/rewrite `post_ctas.buttonUrl` (extracted the single-href rewrite into `rewriteBlogUrl`, shared by both the Tiptap tree-walker and the flat CTA field) so any already-bad CTA link gets caught by the next run of the existing cleanup script.

### Self-hosting hotlinked Pexels images
- Site-health crawls intermittently flagged `/_next/image?url=https://images.pexels.com/...` requests taking 2.4-6s. Confirmed directly from Next's own installed source (`node_modules/next/dist/server/image-optimizer.js`) that a remote image URL goes through `fetchExternalImage` — a real DNS lookup + outbound fetch to the third-party origin before resizing can even start — versus `fetchInternalImage` for a self-hosted file (no outbound request at all). The existing Cloudflare Cache Rule for `/_next/image` and maxed-out `minimumCacheTTL` only help once a URL+width+quality combination is already cached; they don't remove the outbound-fetch cost on a miss, which recurs after every redeploy since the on-disk image-transform cache isn't a persistent Docker volume.
- Root cause: several of the older one-off `scripts/create-*-guide.ts` publish scripts insert directly into `posts` via raw Drizzle with a hardcoded Pexels URL, bypassing `/api/publish`'s already-correct `coverImage.sourceUrl` self-hosting path (fetch → sharp/WebP → `media` row → local `featuredImageUrl`) entirely. Two of these were already manually re-hosted in a prior migration (`scripts/apply-media-sync.ts`) — proving this exact fix has precedent, but ~10 posts were never migrated.
- Added `scripts/audit-hotlinked-images.ts` (read-only, same convention as `scripts/audit-link-targets.ts`) — confirmed 10 posts with a hotlinked `featuredImageUrl` against local dev data (which shares the same seed content as production, since both were populated by the same `create-*-guide.ts` scripts).
- Added `scripts/fix-hotlinked-images.ts` (dry-run by default, `--apply` to write). Couldn't import `lib/media/process-upload.ts`'s `processImageBuffer` directly — that module starts with `import "server-only"`, which only resolves inside Next's own build system, not under plain `tsx` (confirmed: no existing script in this repo imports any `server-only`-marked file). Inlined the exact same sharp recipe (rotate, cap width 2000, WebP quality 82, write to `uploads/`) directly in the script instead.
- Verified the full pipeline for real against local dev (which has matching hotlinked data): ran with `--apply`, confirmed all 10 posts migrated to real self-hosted `/uploads/*.webp` files with correct sizes, re-ran the audit script to confirm 0 hotlinked remained, then reverted local dev back to its original state (restored the 10 posts' original Pexels URLs, deleted the test media rows and files) so this verification pass didn't leave local dev altered. Production still needs this run for real, on the VPS, same pattern as every other data-fix this session.
- Caught before running on production: `fix-hotlinked-images.ts` writes to `./uploads` as an inseparable part of its resize step, but `docker-compose.yml`'s `migrate` service (the container every one-off script this session has run through) had no volume mount for it — only `app` does. Running `--apply` there would have inserted real `media`/`posts` DB rows pointing at `/uploads/<uuid>.webp` while the actual file vanished the instant the throwaway `--rm` container exited, since nothing but the ephemeral container layer ever held it. Added the same `uploads:/app/uploads` mount to the `migrate` service so one-off scripts that touch uploads persist to the same volume `app` serves from.
- All three fixes deployed and verified live: `fix-blog-prefix-links.ts --apply` rewrote 32 links (27 body + 5 CTA, more than the original 27 since this run covers CTAs too) across production — confirmed via a second dry-run reporting `0` remaining. `audit-hotlinked-images.ts` found 14 hotlinked posts on production (vs. ~10 estimated from local dev's matching seed data); `fix-hotlinked-images.ts --apply` migrated all 14 with 0 failures, and two of the new `/uploads/*.webp` files were spot-checked directly (`200`, correct `image/webp` content-type) to confirm they actually persisted to the shared volume rather than the container's ephemeral layer.
...
- Set real `CLOUDFLARE_API_TOKEN`/`CLOUDFLARE_ZONE_ID` values in `.env.production` and confirmed the existing `purgeCloudflareUrls()` plumbing actually purges (direct API test, then a live purge-then-`MISS` check) — previously configured in code but silently no-op'd with no credentials present.
- Added the Cache Rule for these 7 paths: custom filter matching the exact 7 literal paths, `Eligible for cache`, `Ignore cache-control header and use this TTL` (1hr) — deliberately different from the `/_next/image` and category/post Cache Rules (both "respect origin headers"), since these 7 still correctly send `Cache-Control: private, ... must-revalidate` from origin and would cache nothing under that setting. Placed last (order 3) so it overrides the broader, pre-existing "Cache public pages" rule's Edge TTL for just these paths (Cloudflare Cache Rules apply in order, later rules override matching settings from earlier ones — not first-match-wins). Verified all 7 paths transition to `cf-cache-status: HIT` on repeat requests.
- Verified purge-on-publish end-to-end against live production: edited an author's bio in `/admin/authors`, reloaded the public `/authors` page immediately (no wait), and the change appeared instantly despite the 1hr edge TTL — confirms `updateTag("authors")` + `invalidatePublicPaths`'s Cloudflare purge call both fire correctly on the real save path, not just in isolated API tests. This closes out the whole force-dynamic/BYPASS investigation: all 7 pages are now edge-cached and stay fresh on publish.

### Wrong-category post URLs returning a raw 500 instead of redirecting
- A follow-up `run_site_health_check` surfaced 19 internal links (inside posts published the same day via the `app/api/publish/route.ts` agent integration) pointing to `/blog/<slug>` instead of the post's real `/<category>/<slug>` URL. Live testing showed the actual bug is broader: requesting *any* real post's slug under a mismatched category segment (not just the nonexistent `/blog/` prefix) intermittently returns a raw uncaught-exception 500 (no Next.js headers, plain "Internal Server Error", `cf-cache-status: BYPASS`) instead of a 404 or redirect.
- Read `app/(site)/[category]/[slug]/page.tsx`, `lib/posts/get-post-page-data.ts`, and `lib/site-categories.ts` in full — confirmed no unguarded property access anywhere in this app's own component code can be triggered by a category mismatch. `getPostPageData(slug)` looks up posts by slug only and returns the real category as an already-fetched, fully null-guarded value; the URL's category segment is only ever used in plain string interpolation. The crash almost certainly lives inside Next's own on-demand ISR internals for the `generateStaticParams() { return [] }` + `dynamicParams: true` route when the same cached slug data renders under two different first-touch category paths — not something pinpointable further without production server logs.
- Fix: added a canonical-URL guard in the page component — `if (category && category.slug !== categorySlug) permanentRedirect(`/${category.slug}/${slug}`)`. This also fixes a real, always-reproducible SEO defect that existed independently of the crash (wrong-category URLs silently rendering 200 with an incorrect canonical), and means the 19 existing bad `/blog/` links now redirect correctly without needing to touch the stored post content — no tool currently exists to read a post's full body back out before a `replace`-mode republish, so that data fix is deferred.
- Verified `DATABASE_URL="" npx next build` still succeeds and `[category]/[slug]` stays SSG-eligible post-change.
- After deploying (`386f83f`), the redirect itself worked (confirmed clean `308`s for both a fresh mismatched-category combo and the original broken `/blog/` links), but it exposed a second, previously-masked bug: the canonical destination page itself, `/betting-guides/bankroll-management-gaming-guide`, started 500ing *consistently* (not intermittently) on every direct request — worse than the original symptom, since this was the correct category/slug combo with no redirect involved at all. Only this one post crashed; the other posts from the same publish batch rendered fine, ruling out a batch-wide data issue and pointing at this post's own stored fields.
- Root cause: an unguarded `new URL(post.featuredImageUrl, SITE_URL)` call (and the matching one for `author.avatarUrl`) in the page component — thrown for any post whose stored URL isn't well-formed, with no catch anywhere above it in the render tree. This post (published via the same raw-content API as the `/blog/` link batch) had a malformed value in one of those two fields. Wrapped both in `try/catch`, falling back to `null` on failure — same "don't let one bad optional field break the whole page" pattern already used elsewhere (e.g. a trashed author rendering without a byline instead of crashing). Deployed as `a3927f2`; confirmed the canonical URL now returns a clean `200` on 3/3 direct requests, with the wrong-category redirect still working correctly alongside it.
- The redirect made the 19 bad `/blog/<slug>` links harmless but the stored `content` still has the wrong href. Building a proper fix (new authenticated read endpoint + MCP tool to fetch a post's full content before a safe patch) turned out unnecessary — `scripts/fix-link-targets.ts`/`audit-link-targets.ts` (prior session) already establish the right pattern for this class of one-off migration: a plain Drizzle script walking every post's/page's Tiptap `content` tree, dry-run by default, `--apply` to write. Added `scripts/fix-blog-prefix-links.ts` following that exact shape — builds a slug→real-category-URL map from all published posts, rewrites any `/blog/<slug>` link mark whose slug resolves to a real post, leaves anything unresolved untouched and logged rather than guessing. Confirmed clean dry-run against local dev (0 changes, as expected — dev DB doesn't have these posts) and a clean type-check. Production DB is only reachable from the VPS itself (confirmed: `docker-compose.yml` binds Postgres to `127.0.0.1` on the VPS, no SSH tunnel/read-replica exists anywhere in this repo), so running this for real against production is a manual step on the VPS, same as the existing link-target scripts.
